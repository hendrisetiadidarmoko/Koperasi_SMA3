<div class="container mt-4">
    ...
    <button wire:click="generatePdf('{{ $year }}', '{{ $month }}')" class="btn btn-success">
            ...
            <table class="table table-striped table-bordered border-dark rounded-2">
                ...
                <tfoot class="table-light border-dark">
                    <tr>
                        ...
                        <td colspan="3" class="text-end">{{ number_format($grandTotalSell ?? 0, 0, '.', ',') }}</td>
                ...
                <tbody class="table-light border-dark">
                    @forelse ($items as $item)
                        <tr>
                            ...
                            <td>{{$item->total_sell}}</td>
                            <td>{{$item->price_sell?? 0, 0, '.', ','}}</td>
                            <td>{{$item->price_total_sell?? 0, 0, '.', ','}}</td>
                            <td>{{$item->total_RL}}</td>
                            <td>{{$item->price_sell_RL?? 0, 0, '.', ','}}</td>
                            <td>{{$item->price_buy_Rl?? 0, 0, '.', ','}}</td>
                            <td>{{$item->price_total_RL?? 0, 0, '.', ','}}</td>
                            <td>{{$item->total_remaining_stock}}</td>
                            <td>{{ number_format($item->price ?? 0, 0, '.', ',') }}</td>
                            <td>{{$item->price_total_remaining_stock ?? 0, 0, '.', ','}}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="text-center">Tidak ada data tersedia</td>
